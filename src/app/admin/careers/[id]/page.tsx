...
interface CareerAdminDetailPageProps {
  params: Promise<{
    id: string;
  }>;
}

export default function CareerAdminDetailPage({ params }: CareerAdminDetailPageProps) {
  const resolvedParams = use(params);
  const careerId = resolvedParams.id;
  const router = useRouter();

  const { getLocalizedText, locale } = useLanguage();
  const { toast } = useUI();
