...
function ShowcaseEditForm() {
  const searchParams = useSearchParams();
  const slideIdStr = searchParams.get('id') || '';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/admin/homepage/showcase/detail?id=${slideId}`);
    } catch (err: any) {
    ...
    return (
      <AdminLayout title="Edit Showcase Slide">
        <div className="flex items-center justify-center py-20">
          <Spinner size="md" label="Loading showcase slide..." />
        </div>
      </AdminLayout>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-5 pb-12">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 sm:p-5 rounded-lg border border-slate-200 shadow-xs">
          <div className="flex items-center gap-3">
            <Link
              href={`/admin/homepage/showcase/detail?id=${slideId}`}
