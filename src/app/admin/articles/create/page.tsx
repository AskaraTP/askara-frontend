...
export default function CreateArticlePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validation.isValid) {
      ...
      toast('Please review the form errors before publishing.', 'error');
      return;
    }
    ...
    try {
      ...
      toast(formatErrorMessage(err, 'Failed to publish article'), 'error');
      setSaving(false);
    }
  };
    ...
    <AdminLayout title="Add New Article">
      <form onSubmit={handleSubmit} className="space-y-6 pb-16">
        ...
        <AdminPageHeader
          ...
          subtitle="Publish laboratory insights, scientific articles, or company announcements"
          saving={saving}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <FormField label="Article Title (English)" required error={errors.title_en}>
              <input
                ...
                placeholder="e.g. Modern Rapid Testing in Food Safety"
            ...
            <FormField label="Article Title (Indonesian)" required error={errors.title_id}>
              <input
                ...
                placeholder="e.g. Pentingnya Rapid Testing dalam Keamanan Pangan Modern"
                className="w-full px-3.5 py-2.5 rounded-md border border-slate-200 text-xs sm:text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-colors"
              />
            </FormField>
          </div>
